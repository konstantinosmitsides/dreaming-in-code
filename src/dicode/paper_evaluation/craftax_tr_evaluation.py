...
def make_evaluate(config, env, env_params, num_envs, num_steps):
	def evaluate(train_state, rng):
		...
		def get_stats(data_array, name):
			...
			return {
				f"mean_{name}": jnp.where(count_finished > 0, jnp.nanmean(data_array), -jnp.inf),
				f"median_{name}": jnp.where(count_finished > 0, jnp.nanmedian(data_array), -jnp.inf),
				f"min_{name}": jnp.where(count_finished > 0, jnp.nanmin(data_array), -jnp.inf),
				f"max_{name}": jnp.where(count_finished > 0, jnp.nanmax(data_array), -jnp.inf),
			}

		final_rewards = jnp.where(finished_mask, final_raw_rewards, -jnp.inf)
		final_lengths = jnp.where(finished_mask, final_raw_lengths, -1)
		metrics = {}
		metrics["returns"] = final_rewards
		metrics["lengths"] = final_lengths
		metrics.update(get_stats(rewards_for_stats, "return"))
		metrics.update(get_stats(lengths_for_stats, "length"))
		metrics["completion_rate"] = finished_mask.mean()

		# --- B. Correlation: Length vs Return ---
		# Calculate Pearson correlation manually to handle the mask efficiently
		mu_r = jnp.nanmean(rewards_for_stats) # Mean of rewards
		mu_l = jnp.nanmean(lengths_for_stats) # Mean of lengths
		
		# Center variables and mask unfinished ones to 0.0
		r_centered = jnp.where(finished_mask, final_raw_rewards - mu_r, 0.0)
		l_centered = jnp.where(finished_mask, final_raw_lengths - mu_l, 0.0)
		
		numerator = jnp.sum(r_centered * l_centered)
		denominator = jnp.sqrt(jnp.sum(r_centered ** 2) * jnp.sum(l_centered ** 2))
		
		correlation = jnp.where(
			(count_finished > 1) & (denominator > 1e-6), 
			numerator / denominator, 
			0.0
		)
		metrics["correlation_length_return"] = correlation
...
def main(config, rng, num_envs, num_steps, train_state=None, eval_embedding=None):
	...
	evaluate_fn = jax.jit(make_evaluate(config, env, env_params, num_envs, num_steps))

	metrics = evaluate_fn(train_state, rng)
