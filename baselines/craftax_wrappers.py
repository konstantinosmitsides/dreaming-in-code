...
class LogWrapper(UnderspecifiedEnv):
    ...
    # `params` is deliberately NOT static. Craftax's EnvParams is a flax struct.dataclass whose
    # fields are pytree children, so under an enclosing jit it arrives as tracers and cannot be
    # hashed. Neither this method nor step_env uses params in Python-level control flow -- both
    # pass it straight through -- so tracing it rather than baking it in changes no computation.
