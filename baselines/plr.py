"""Prioritized Level Replay (PLR) and Domain Randomization (DR) on Craftax, with PPO-GTrXL.
...
DR is this same script with the replay mechanism disabled. Run from the repository root:

    python -m baselines.plr --seed <SEED>                      # PLR
    python -m baselines.plr --seed <SEED> --replay_prob 0.0    # DR
"""
...
def sample_trajectories(
    ...
    def sample_step(carry, _):
        rng, train_state, obs, env_state, memories, memories_mask, memories_mask_idx, done, step_env_currentloop = carry
        ...
        carry = (rng, train_state, obsv, env_state, memories, memories_mask, memories_mask_idx, done, step_env_currentloop)
        return carry, (transition, memories_out)

    (rng, train_state, last_obs, last_state, last_memories, last_mask, last_mask_idx, last_done, last_step_env_currentloop), (traj_batch, memories_batch) = jax.lax.scan(
        sample_step,
        (rng, train_state, init_obs, init_env_state, init_memories, init_mask, init_mask_idx, init_done, init_step_env_currentloop),
        None,
...
def main(config=None, project="JAXUED_TEST"):
    ...
    env_params = default_env.default_params
    ...
    def create_train_state(rng) -> TrainState:
        ...
        # 2. Generate a sample level (returns Tuple[Map, Params])
        # init_map, init_params = sample_random_level(rng_level)
        
        # 3. Pass UNPACKED arguments to reset_to_level
        # Map goes to 'level', Params goes to 'params'
        ...
        network_params = network.init(rng, init_memory, init_obs, init_mask)
        tx = optax.chain(
    ...
    def train_step(carry: Tuple[chex.PRNGKey, TrainState], _):
        ...
        def on_new_levels(rng: chex.PRNGKey, train_state: TrainState):
            ...
            # Rollout
            (
                (rng, train_state, last_obs, last_env_state, last_memories, last_mask, last_mask_idx, last_done, _),
                (obs, actions, rewards, dones, log_probs, values, info, advantages, targets, losses, grads)
        ...
        #     parent_maps, parent_params = parent_levels
        ...
        #     child_maps = jax.vmap(mutate_level, (0, 0, None))(
        #         jax.random.split(rng_mutate, config["num_train_envs"]), 
        #         parent_maps, 
        #         config["num_edits"]
        #     )

        #     # We keep parent parameters for the child
        #     child_params = parent_params 
        #     child_levels = (child_maps, child_params)
        ...
        #     init_obs, init_env_state = jax.vmap(env.reset_to_level, in_axes=(0, 0, 0))(
        #         jax.random.split(rng_reset, config["num_train_envs"]), 
        #         child_maps, 
        #         child_params
        #     )
    ...
    seed = config["seed"]
    os.makedirs("craftax_params", exist_ok=True)
    if config["final_checkpoint"] and config["replay_prob"] == 0:
        jnp.save(f"craftax_params/dr_params_{seed}.npy", runner_state[1].params)
    elif config["final_checkpoint"]:
        jnp.save(f"craftax_params/plr_params_{seed}.npy", runner_state[1].params)
...
if __name__=="__main__":
    ...
    mut_group.add_argument("--num_updates", type=int, default=240)
    ...
    # === Eval CONFIG ===
    parser.add_argument("--n_eval_levels", type=int, default=100)
